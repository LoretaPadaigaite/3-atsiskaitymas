...
{
  type VienoSkaiciausFunkcija = (number: number) => number;
  type VienoStringoFunksija = (str: string) => void;
  type DviejuSkaiciuFunkcija = (num1: number, num2: number) => number;

  const multiply: DviejuSkaiciuFunkcija = (a, b) => {
    return a * b;
  };

  const power: VienoSkaiciausFunkcija = function (base, power) {
    return base ** power;
  };

  const squareRoot: VienoSkaiciausFunkcija = (number) => number ** (1 / 2);

  const root: DviejuSkaiciuFunkcija = function (base, nthRoot) {
    return base ** (1 / nthRoot);
  }

  const printBlueText: VienoStringoFunksija = (text) => console.log(`%c${text}`, 'color: #0000ee');
