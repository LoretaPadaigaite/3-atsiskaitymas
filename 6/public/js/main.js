...
console.group('3. Parašykite funkciją, kuri grąžina vienarūšių primityvių reikšmių masyvo kopiją');
{
    ...
    console.log({ numbers, result: arrayCopy(numbers) });
    console.log({ strings, result: arrayCopy(strings) });
    console.log({ booleans, result: arrayCopy(booleans) });
}
console.groupEnd();
...
{
    const mergeTheSameTypeArray = (arr1, arr2) => {
        return [...arr1, ...arr2];
    };
    const args1 = [[1, 2, 3], [4, 5, 6]];
    const args2 = [['nėra', 'blogo', 'oro'], ['tik', 'netinkama', 'apranga']];
    const args3 = [[false, true, true], [false, true, false]];
...
console.group('6. Parašykite funkciją, kuri priimtų bet kokią reikšmę ir grąžintų objektą su savybėmis-funkcijomis "setValue" - reikšmei nustatyti ir "getValue" tai reikšmei gauti. Funkcijai perduota reikšmė neturi būti pasiekiama tiesiogiai.');
{
    ...
    const value1 = 7;
    const value2 = ["Berlynas", "Vokietijos", "Sostinė"];
    const value3 = { name: 'Hortenzija', surname: 'Daugiažiedė' };
