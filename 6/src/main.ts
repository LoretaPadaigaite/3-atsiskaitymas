...
{
  const firstElementOfArray = <Type>(arr: Type[]): Type | undefined => {
    return arr[0];
  }

  console.log({ numbers, result: firstElementOfArray(numbers) });
  console.log({ strings, result: firstElementOfArray(strings) });
  console.log({ booleans, result: firstElementOfArray(booleans) });
}
console.groupEnd();
...
{
  const arrayCopy = <Type extends PrimitiveType>(arr: Type[]): Type[] => {
    const copy: Type[] = arr.map(x => x);

    return copy;
  }

  console.log({ numbers, result: arrayCopy<number>(numbers) });
  console.log({ strings, result: arrayCopy<string>(strings) });
  console.log({ booleans, result: arrayCopy<boolean>(booleans) });
}
console.groupEnd();
...
console.group('5. Parašykite funkciją, kuri sujungia tokių pat tipų masyvus į vieną masyvą');
{
  ...
  const mergeTheSameTypeArray = <Type>(arr1: Type[], arr2: Type[]): Type[] => {
    return [...arr1, ...arr2];
  }

  const args1: ArgumentSample<number> = [[1, 2, 3], [4, 5, 6]];
  const args2: ArgumentSample<string> = [['nėra', 'blogo', 'oro'], ['tik', 'netinkama', 'apranga']];
  const args3: ArgumentSample<boolean> = [[false, true, true], [false, true, false]];

  console.log({ args: args1, result: mergeTheSameTypeArray(...args1) });
  console.log({ args: args2, result: mergeTheSameTypeArray(...args2) });
  console.log({ args: args3, result: mergeTheSameTypeArray(...args3) });
}
console.groupEnd();
...
console.group('6. Parašykite funkciją, kuri priimtų bet kokią reikšmę ir grąžintų objektą su savybėmis-funkcijomis "setValue" - reikšmei nustatyti ir "getValue" tai reikšmei gauti. Funkcijai perduota reikšmė neturi būti pasiekiama tiesiogiai.');
{
  ...
  const value2: Array<string> = ["Berlynas", "Vokietijos", "Sostinė"];
  const value3: { name: string, surname: string } = { name: 'Hortenzija', surname: 'Daugiažiedė' };
